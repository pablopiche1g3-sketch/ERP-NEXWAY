...
CREATE TABLE IF NOT EXISTS public.client_credit_scorings (id uuid DEFAULT uuid_generate_v4() PRIMARY KEY, client_id uuid REFERENCES public.clients(id) ON DELETE CASCADE NOT NULL, score numeric(5,2) DEFAULT 90.00, risk_level text DEFAULT 'BAJO', avg_pay_days integer DEFAULT 15, recommended_limit numeric(10,2) DEFAULT 5000.00, updated_at timestamptz DEFAULT timezone('utc'::text, now()) NOT NULL);
CREATE TABLE IF NOT EXISTS public.customer_advances (id uuid DEFAULT uuid_generate_v4() PRIMARY KEY, client_id uuid REFERENCES public.clients(id) ON DELETE CASCADE NOT NULL, amount numeric(10,2) NOT NULL DEFAULT 0.00, used_amount numeric(10,2) NOT NULL DEFAULT 0.00, notes text, created_at timestamptz DEFAULT timezone('utc'::text, now()) NOT NULL);

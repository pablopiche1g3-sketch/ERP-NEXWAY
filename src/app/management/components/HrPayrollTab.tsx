...
export default function HrPayrollTab() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error(e);
      toast({ variant: 'destructive', title: 'Error', description: 'No se pudo cargar perfiles para RH.' });
    } finally {
      setLoading(false);
    }
  };
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-100 dark:bg-white/5 border-b border-slate-200 dark:border-white/10">
                    <tr className="text-left text-slate-500 dark:text-slate-400">
                      <th className="p-4 font-bold text-xs uppercase">Empleado (Email)</th>
                  ...
                  <tbody>
                    {users.map(u => (
                      <tr key={u.id} className="border-b border-slate-100 dark:border-white/5 last:border-0 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                        <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">{u.email}</td>
                        <td className="p-4"><span className="uppercase text-[10px] font-black text-indigo-500 bg-indigo-500/10 px-2 py-1 rounded-md tracking-widest">{u.role}</span></td>
                        <td className="p-4">
                          <Input type="number" placeholder="0.00" className="w-32 h-9 text-sm font-bold bg-white dark:bg-black/20" />
                        </td>
                        <td className="p-4 text-right">
                          <Button size="sm" variant="ghost" className="h-9 font-bold text-emerald-600 hover:text-emerald-700 hover:bg-emerald-500/10">

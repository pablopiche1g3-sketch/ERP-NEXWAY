...
const MODULE_BLUEPRINTS: Record<string, string> = {
  'Facturación y Ventas': 'Pestañas: 1. POS (Punto de Venta) [TOUR:tab-pos]. 2. Historial de Ventas (Ventas pasadas, tickets y anulación) [TOUR:tab-history]. 3. Arqueo de Caja (Cierre ciego y ajustes) [TOUR:tab-arqueo]. 4. Créditos (CXC) [TOUR:tab-creditos]. 5. Configuración [TOUR:tab-config]. IMPORTANTE: Al corregir un método de pago, el sistema genera automáticamente un ajuste dual (registro inalterable en audit_logs y asiento de ajuste en el Libro Diario/journal) para mantener la contabilidad 100% cuadrada.',
  ...
  'Registro de Compras': 'Funciones principales: Ingresar facturas de proveedores, alimentar el inventario con nuevas compras (afecta el stock positivamente) y registrar cuentas por pagar (CXP). Pestañas: 1. Historial Compras [TOUR:tab-historial]. 2. Órdenes [TOUR:tab-ordenes]. 3. Bandeja Gmail [TOUR:tab-gmail] para importar DTEs.',
  ...
  'CRM Comercial': 'Funciones: Pipeline de ventas, tablero Kanban de oportunidades y Mapa Logístico. Pestañas: 1. Embudo Kanban [TOUR:tab-kanban]. 2. Mapa Logístico (Recomendaciones y clientes geolocalizados) [TOUR:tab-mapa]. 3. Bandeja CRM (Gmail) [TOUR:tab-gmail] para correos y seguimiento.',
  'Gerencia y Reportes': 'Pestañas: 1. Dashboard de KPIs. 2. Gestión de Sucursales y Cajas. 3. Usuarios y Roles (RBAC, crear cajeros y admins). 4. Auditoría Gmail [TOUR:tab-gmail] para monitorear correos de la empresa.',
